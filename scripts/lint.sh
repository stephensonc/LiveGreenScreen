...
if [[ "$OS" == "Darwin" ]]; then
    FILES=$(find -E . -type f -regex '\./src/.*.py')
else
    FILES=$(find . -type f -regextype posix-extended -regex '\./src/.*.py')
fi
...
declare -A LINTERS
LINTERS=( ["black"]="pipenv run black $CHECK $FILES" ["pylint"]="pipenv run pylint $FILES" ["flake8"]="pipenv run flake8 $FILES" )
